...
#Plan of record = 728,TO Be defined = 609,Draft = 610,Under review = 295,Rejected = 597,To be resolved = 292,Approved = 293
#
Req_Status_dict = {292:'To Be Resolve', 293:'Approved', 295:'Under Review',597:'Rejected', 609:'To Be Defined', 610:'Draft',728:'Plan of Record'}
#jama_client = 0
#################################################################################################################################
...
def Update_VC(Api_item,Ver_Cri):
    ...
    """try:
        ...
        if Debug == 'enable':
            sg.Print("Either Link Already Exists or check Jama credentials")"""
################################################################################################################################
################## Delete Upstream relationship for MB - individual item ###########################
def DeleteMultipleLink(Del_item,excelpath):

    excel_obj = openpyxl.load_workbook(excelpath)
    ...
    for i in range(4, row + 1):     #RANGE CAN BE CHANGED
        ...
        #read 1 column
        cell_obj = sheet_obj.cell(row = i, column = int(1))   # TO item column number MAY CHANGE
        Del_item = cell_obj.value
        sg.Print(" API ID =",Del_item)

        if Del_item == None:
            break
        else:  
            Delete_Link(Del_item)
            continue


    sg.Print(" Number of items updated =",i)
    return 1


################## Delete Upstream relationship for MB - individual item ###########################
def Delete_Link(ApiDel_item):
    global client_ID
    global client_code
    jama_client3 = JamaClient('https://luminar.jamacloud.com', credentials=(client_ID, client_code), oauth=True)


    Uprela = jama_client3.get_items_upstream_relationships(ApiDel_item,1)
    #sg.Print(Uprela)
    ...
    try:
        ...
        '''for i in range(3):
        sg.Print(Uprela[i]['toItem'])
        items0 = jama_client3.get_item(Uprela[i]['toItem'])'''

        for i in range(0, len(Uprela) ):
                    
            items0 = jama_client3.get_item(Uprela[i]['fromItem'])

            #Relation_Id = Uprela[0]['id']
            #sg.Print(items0)

            sg.Print("Index =",i,"Relationship ID =",Uprela[i]['id'])
            if 'DAIM-DAIM-' in items0["documentKey"]:   #For MB

            #if 'Sys_In-SYRQ' in items0["documentKey"]:
            #if 'Sys_In-SYRQ' in items0["documentKey"]:
                #sg.Print(items0["documentKey"])

                try:
                    sg.Print("From Item API=",Uprela[i]['fromItem'],"To Item API =",Uprela[i]['toItem'])
                    DelRela_items = jama_client3.delete_relationships(Uprela[i]['id'])  
                    sg.Print(DelRela_items)

                    TagNew_items = jama_client3.post_item_tag(Uprela[i]['fromItem'],tag_id=934)
                    sg.Print(TagNew_items)
                except:
                    sg.popup(title="Error",custom_text="!! Uplink already Deleted/None Present !!")
                    sg.Print("!! Uplink already Deleted/None Present !! API ID=",Uprela[i]['toItem'])


    except:
        sg.Print("No uplink found API ID=",ApiDel_item)
################## Add a tag  - individual item ###########################
'''def add_a_tag(Tag_id):      

    jama_client4 = JamaClient('https://luminar.jamacloud.com', credentials=(client_ID, client_code), oauth=True)
    # CHANGE_TO_EVIDENCE id=986 
    TagNew_items = jama_client4.post_item_tag(Uprela[0]['fromItem'],tag_id=934)
    sg.Print(TagNew_items)'''
...
# Define your JAMA API endpoint and authentication credentials
api_url = "https://your-jama-api-url.com/rest/v1/requirements"
headers = {
    "Authorization": "Bearer your_api_token",
    "Content-Type": "application/json"
}

# Make a GET request to fetch requirements data from JAMA
response = requests.get(api_url, headers=headers)

if response.status_code == 200:
    requirements_data = response.json()
    
    # Calculate KPIs based on the requirements data
    total_requirements = len(requirements_data)
    # Add your KPI calculation logic here based on your specific requirements and metrics
    
    # Example: Calculate the percentage of requirements that are approved
    approved_requirements = [req for req in requirements_data if req.get('status') == 'Approved']
    approved_percentage = (len(approved_requirements) / total_requirements) * 100
    
    print(f"Total Requirements: {total_requirements}")
    print(f"Percentage of Approved Requirements: {approved_percentage}%")
    
else:
    print("Failed to fetch requirements data from JAMA. Status code:", response.status_code)'''
...
def kpi(api_new):
    ...
    #Api_item = 2270822
    jdata = jama_client4.get_item(api_new)

    sg.Print(" Item =",api_new)
    sg.Print(" Status =",jdata['fields']['status'])


    match jdata['fields']['status']:
        case 292:
            sg.Print(" Requirement status =",Req_Status_dict[292])
        case 293:
            sg.Print(" Requirement status =",Req_Status_dict[293])
        case 295:
            sg.Print(" Requirement status =",Req_Status_dict[295])
        case 597:
            sg.Print(" Requirement status =",Req_Status_dict[597])
        case 609:
            sg.Print(" Requirement status =",Req_Status_dict[609])
        case 610:
            sg.Print(" Requirement status =",Req_Status_dict[610])
        case 728:
            sg.Print(" Requirement status =",Req_Status_dict[728])
        case _:
            sg.Print(" status not found")
...
def win3():
    ...
    [sg.Combo(list,default_value=0, s=(15,22), enable_events=True, readonly=True, k='-COMBO-'),sg.Button("XXX"),],
    ...
    [sg.Button("Update"), sg.Button('Exit'), sg.Button('Delete UpLinks')]
    ]

    # Window size (x, y)  x = 950 horizontal and y = 250 vertical
    window2 = sg.Window('Verification criteria', layout, size=(950, 300))
    ...
    while True:
        ...
        try:
            ...
            if Data['-COMBO-'] == "STOP":
                window2['-Verification_criteria-'].update(disabled=True)
            else:
                window2['-Verification_criteria-'].update(disabled=False)

            if event == 'XXX':
                # read API ID Jama item and update verification criteria
                #if Data['-COMBO-'] is "STOP":
                    #window2['-Verification_criteria-'].update(disabled=True)
                sg.Print("Reserved for Future Release",Data['-COMBO-'],text_color='white',background_color='red')
                logOutput.write("Reserved for Future Release")
                logOutput.write(Data['-COMBO-'])
                logOutput.write("\n")
            if event == 'Update':
            ...
            if event == 'Delete UpLinks':

                if Data['Single'] == True:
                    # read API ID Jama item
                    sg.Print("Deletion of uplink started Started")
                    Delete_Link(Data['-API_ID-'])
                else:
                    # read API ID Jama item
                    sg.Print("Deletion of Multiple Relationship uplink Started")
                    success = DeleteMultipleLink(Data['-API_ID-'],Data['-PATH-'])
                    sg.Print("Operation successfull =",success)
        except:
            sg.Print("Window Closed")
            logOutput.write("Window Closed \n")
    window2.close()
    logOutput.write("Window Closed \n")
...
logOutput.write("Logging Started \n")
settings = json.load(f)
...
#       sg.popup_no_buttons('Reserved for future implementations',title='INFO')
    ...
    if event == 'About...':
        #print('Current vserion is 1.9')
        sg.popup_no_buttons("Delete a relationships from list of API IDs and Also single ID ; Added log output file;Added dummy dropdown",text_color="Yellow",title="Revision 1_9",non_blocking=True)
